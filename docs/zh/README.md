...
更多详情请查阅 [**`docs/zh/SELF_HOSTING.md`**](/docs/zh/SELF_HOSTING.md)。
...
2. **启动开发环境**:
   ...
   npm run dev
   ```
   这会同时启动服务器（端口 8544）和客户端（端口 8543），支持热重载。

3. **完成设置并开始开发**:
   打开 `http://localhost:8543/install` 以完成网页版设置。对于任何基础设施的自定义（如端口），请创建一个 `.env.local` 文件。
