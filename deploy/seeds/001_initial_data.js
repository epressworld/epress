/**
 * @param { import("knex").Knex } knex
 * @returns { Promise<void> }
 */

const defaultAvatar =
  "data:image/png;base64,"
export async function seed(knex) {
  console.log("🌱 Starting to populate initial data...")

  try {
    // 检查是否已存在 is_self=true 的节点记录，确保幂等性
    const selfNode = await knex("nodes").where({ is_self: true }).first()

    if (!selfNode) {
      console.log(
        "No self node found. Seeding initial self node data and settings...",
      )

      const ethereumAddress = process.env.EPRESS_NODE_ADDRESS
      const nodeUrl = process.env.EPRESS_NODE_URL
      const nodeTitle = process.env.EPRESS_NODE_TITLE || "My Epress Node"
      const nodeDescription =
        process.env.EPRESS_NODE_DESCRIPTION ||
        "A decentralized blog powered by Epress."

      if (!ethereumAddress || !nodeUrl) {
        throw new Error(
          "Missing required environment variables for initial node setup: NODE_ETHEREUM_ADDRESS and NODE_URL must be set.",
        )
      }

      // 插入 is_self=true 的节点记录
      await knex("nodes").insert({
        address: ethereumAddress,
        url: nodeUrl,
        title: nodeTitle,
        description: nodeDescription,
        is_self: true,
        profile_version: 0,
        created_at: new Date(),
      })
      console.log("✅ Self node record inserted.")

      // 插入 settings 记录
      const defaultSettings = [
        { key: "allow_comment", value: "true" },
        { key: "enable_rss", value: "true" },
        { key: "allow_follow", value: "true" },
        {
          key: "avatar",
          value: defaultAvatar,
        }, // Existing default avatar
      ]

      for (const setting of defaultSettings) {
        // 检查设置项是否已存在，确保幂等性
        const existingSetting = await knex("settings")
          .where({ key: setting.key })
          .first()
        if (!existingSetting) {
          await knex("settings").insert(setting)
          console.log(`✅ Default node setting '${setting.key}' inserted.`)
        } else {
          console.log(
            `ℹ️ Node setting '${setting.key}' already exists. Skipping.`,
          )
        }
      }
    } else {
      console.log("Self node already exists. Skipping initial data setup.")
    }
  } catch (error) {
    console.error("❌ Seed data population failed:", error)
    throw error
  }
}
